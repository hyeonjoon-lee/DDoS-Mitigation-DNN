...
"""
from pox.core import getLogger, core
...
class l3_switch(EventMixin):
    ...
    def _handle_openflow_PacketIn(self, event):
        ...
        if isinstance(packet.next, ipv4):
            ...
            input_packet = [len(packet), packet.next.hl, packet.next.iplen, 0, ip_flag_df, ip_flag_mf, packet.next.frag, packet.next.ttl, packet.next.protocol, packet.next.next.srcport, packet.next.next.dstport, len(packet.next.next), packet.next.next.ACK, 0, 0, 0, 0, 0, tcp_flag_ack, tcp_flag_push, tcp_flag_reset, tcp_flag_syn, tcp_flag_fin, packet.next.next.win, 0]
            global packets_arr
            packets_arr.append(input_packet)

            if len(packets_arr) > 25:
                packets_arr.pop(0)

                model = load_model("/home/ubuntu/pox/pox/forwarding/model_25_features_hping3.h5")
                scalar = pickle.load(open('/home/ubuntu/pox/pox/forwarding/myScalar.pkl', 'rb'))
